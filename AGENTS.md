...
- **`platform.json`:** Service metadata intended for `homelab-platform`. Nothing currently reads it at runtime — treat its `environment` list as aspirational/stale documentation (see `task.md`).
- **CI/CD:** Push to `main` → GitHub-hosted runner (`ubuntu-latest`) → builds Docker image → pushes to `ghcr.io/collani-homelab/homelab-mcp:latest` + `:<sha>`. This is working and automatic. The old self-hosted runner workflow is gone.
- **Deploy cadence:** The `image-updater` Dagu DAG in `homelab-platform` runs nightly at 3am, pulls the latest image from ghcr.io, and calls `homelab-deploy -deploy homelab-mcp`. The SRE machine's running container is updated overnight. For an immediate restart without waiting, use the `redeploy_service` MCP tool (see Section 6).
- **Secrets:** Bound via `.env` on the SRE machine at `~/repos/homelab-mcp/.env`. Never committed. `IMAGE_TAG` must be `ghcr.io/collani-homelab/homelab-mcp:latest` — without it, docker compose falls back to `homelab-mcp:latest` and tries Docker Hub.
- **Docker auth:** The SRE machine's Docker daemon must be authenticated to ghcr.io with `read:packages` scope. Refresh with: `gh auth token | docker login ghcr.io -u wcollani --password-stdin`.
...
- **Agents must not do manual deploy flows** unless the user explicitly asks. Make changes, commit, push — CI builds the image. The nightly DAG deploys it. Use `redeploy_service` only for force-restarts without a code change.
